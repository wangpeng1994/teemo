...
public class ElasticsearchDataGenerator {
    ...
    private static void writeSingleThread(List<News> newsFromMySQL) {
        try (RestHighLevelClient client = new RestHighLevelClient(
            ...
            // write 2000*1000 = 200_0000 records use single thread.
            for (int i = 0; i < 1000; i++) {
                ...
                for (News news : newsFromMySQL) {
                    ...
                    data.put("content", news.getContent().length() > 50 ? news.getContent().substring(0, 50) : news.getContent());
    ...
    private static List<News> getNewsFromMySQL(SqlSessionFactory sqlSessionFactory) {
        try (SqlSession session = sqlSessionFactory.openSession()) {
            return session.selectList("com.github.wangpeng1994.MockMapper.selectNews");
        }
    }
